...
class CacheService {
  constructor() {
    ...
    this.suppressWarnings = false; // Suppress repeated "not ready" warnings
  }

  // Initialize Redis connection
  async connect() {
    // Skip Redis if no URL configured - use in-memory fallback
    if (!process.env.REDIS_URL && !process.env.REDIS_HOST) {
      logger.info('Redis not configured - using in-memory cache fallback');
      this.isConnected = false;
      this.suppressWarnings = true; // Don't spam logs about Redis not being ready
      return;
    }
  ...
  async set(key, value, ttl = this.defaultTTL) {
    if (!this.isReady()) {
      if (!this.suppressWarnings) logger.debug('Redis not ready, skipping cache set');
      return false;
    }
  ...
  async get(key) {
    if (!this.isReady()) {
      if (!this.suppressWarnings) logger.debug('Redis not ready, skipping cache get');
      return null;
    }
  ...
  async del(key) {
    if (!this.isReady()) {
      if (!this.suppressWarnings) logger.debug('Redis not ready, skipping cache delete');
      return false;
    }
  ...
  async delPattern(pattern) {
    if (!this.isReady()) {
      if (!this.suppressWarnings) logger.debug('Redis not ready, skipping cache pattern delete');
      return false;
    }
