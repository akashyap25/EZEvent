...
const analyzeReviews = async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ event: eventId }).select('rating review title');
    
    if (reviews.length === 0) {
      return success(res, { sentiment: { summary: 'No reviews yet', averageRating: 0, totalReviews: 0 } }, 'No reviews to analyze');
    }
