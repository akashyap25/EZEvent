...
router.get('/:eventId/attendees',
  ...
  async (req, res) => {
    try {
      ...
      // Get attendees from orders
      const orders = await Order.find({ eventId: req.params.eventId, status: 'confirmed' })
        .populate('userId', 'firstName lastName email avatar');
      const attendees = orders.map(o => o.userId).filter(Boolean);
