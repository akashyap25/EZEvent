...
// Organizer sends custom email to event attendees
router.post('/event/:eventId/custom',
  authenticateToken,
  requireAuth,
  async (req, res) => {
    try {
      ...
      // Only organizer or co-organizer can send
      if (event.organizer.toString() !== userId) {
        const isCoOrg = event.coOrganizers?.some(co => co.user?.toString() === userId);
        if (!isCoOrg) {
          return res.status(403).json({ success: false, message: 'Only the organizer can send emails to attendees' });
        }
      }

      if (!event.attendees || event.attendees.length === 0) {
        return res.status(400).json({ success: false, message: 'No attendees to email' });
      }

      const attendees = await UserModel.find({ _id: { $in: event.attendees } }).select('email firstName');
      const organizer = await UserModel.findById(event.organizer).select('firstName lastName');
      ...
      for (const attendee of attendees) {
        try {
          await EmailService.sendCustomOrganizerEmail(attendee.email, {
      ...
      res.json({
        success: true,
        message: `Email sent to ${sent} attendees${failed > 0 ? `, ${failed} failed` : ''}`,
        data: { sent, failed, total: attendees.length }
      });
    } catch (error) {
