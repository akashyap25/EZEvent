...
- [x] For each of the 7 flagship features: expand automated tests, fix rough edges, manually verify the full happy path + 2-3 edge cases end-to-end. *(Payments and QR check-in done — see below. Chat, SMS, Push, Organizations, Search/Analytics remaining.)*
...
- [x] Payments: verify Stripe webhook idempotency in `orderController.js`'s `handleStripeWebhook` — duplicate webhook delivery should not double-process an order. **Done**: fixed a real check-then-create race condition (unique index on `stripeId` + atomic create/duplicate-key handling), removed an insecure direct order-creation endpoint that bypassed payment entirely, fixed orders never reaching `status:'completed'`, isolated the confirmation email from the main try/catch, and root-caused + fixed why Stripe webhooks would fail signature verification in production (raw body was being clobbered by duplicate middleware). New `backend/tests/orders.test.js`.
- [x] QR check-in: **Done** (moved up from later, found during payments work) — fixed a broken (not just insecure) ownership check in `generateTicketQR` (`req.user.userId` was always undefined, feature was completely non-functional), and added missing organizer-only authorization to `scanCheckIn`/`manualCheckIn`/`getEventCheckIns`/`getCheckInStats`/`undoCheckIn` (any authenticated user could previously check in any attendee for any event). New `backend/tests/checkin.test.js`.
